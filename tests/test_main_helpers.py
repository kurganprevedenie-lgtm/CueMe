"""Тесты чистых хелперов бота (без сети/aiogram-раннера).

После разбора main.py по модулям хелперы живут в handlers/common.py и
handlers/reply_flow.py — импорт этих модулей поднимает Router, но не Bot."""
from handlers import common, reply_flow


# ── _contact_name ─────────────────────────────────────────────────────────────

def test_contact_name_variants():
    assert common._contact_name({"display_name": "Аня", "username": "anya", "contact_alias": "x"}) == "Аня (@anya)"
    assert common._contact_name({"display_name": "", "username": "anya", "contact_alias": "x"}) == "@anya"
    assert common._contact_name({"display_name": "Аня", "username": "", "contact_alias": "x"}) == "Аня"
    # ни имени, ни юзернейма → алиас
    assert common._contact_name({"display_name": "", "username": "", "contact_alias": "alias-uuid"}) == "alias-uuid"


def test_contact_name_without_username_column():
    # строка без колонки username не должна падать (ветка "username" in keys)
    assert common._contact_name({"display_name": "Маша", "contact_alias": "a"}) == "Маша"


# ── _split_long_text ──────────────────────────────────────────────────────────

def test_split_long_text_short_stays_single():
    assert common._split_long_text("коротко") == ["коротко"]


def test_split_long_text_respects_limit_and_covers_content():
    text = "\n\n".join("абв" * 4 for _ in range(6))  # 6 абзацев по 12 символов
    parts = common._split_long_text(text, limit=20)
    assert len(parts) > 1
    assert all(len(p) <= 20 for p in parts)
    joined = "".join(p.replace("\n", "") for p in parts)
    assert joined == text.replace("\n", "")


# ── _style_cache_key ──────────────────────────────────────────────────────────

def test_style_cache_key_deterministic_and_sensitive():
    k1 = common._style_cache_key("reply", "flirt", "привет", "card-A", "inter-A")
    k2 = common._style_cache_key("reply", "flirt", "привет", "card-A", "inter-A")
    assert k1 == k2 and len(k1) == 64
    # смена карточки стиля меняет ключ (авто-инвалидация)
    assert common._style_cache_key("reply", "flirt", "привет", "card-B", "inter-A") != k1
    assert common._style_cache_key("rewrite", "flirt", "привет", "card-A", "inter-A") != k1
    # смена пола (extra) — тоже меняет ключ (иначе после /gender вернётся кэш старого рода)
    assert common._style_cache_key("reply", "flirt", "привет", "card-A", "inter-A", extra="male") != k1
    assert (
        common._style_cache_key("reply", "flirt", "привет", "card-A", "inter-A", extra="male")
        != common._style_cache_key("reply", "flirt", "привет", "card-A", "inter-A", extra="female")
    )


# _last_incoming_line удалена вместе с функцией «скриншот переписки → ответ»
# (handlers/reply_flow.py — секция «Ответить по скриншоту», закомментирована).


# _auto_style_for_ctx удалена вместе со старой style_pick_kb-инфраструктурой
# (точечный выбор стиля/«Другой тон» убраны — см. handlers/reply_flow.py).


# ── _reply_data_signals ───────────────────────────────────────────────────────

def test_reply_data_signals_stage_from_totals_plus_situation():
    samples = {
        "features_summary": "Пользователь: 200 сообщ., длина 40. Собеседник: 190 сообщ., длина 30.",
        "my_sample": ["a"], "contact_sample": ["b"],   # длины малы, но тоталы из сводки главнее
    }
    sig = reply_flow._reply_data_signals(samples, "ок")
    assert sig is not None
    assert "давняя переписка" in sig          # стадия по реальным тоталам (390)
    assert "сух" in sig                        # сухая реплика «ок»


def test_reply_data_signals_none_when_nothing():
    # нет семплов и обычная реплика → сигналов нет
    assert reply_flow._reply_data_signals(None, "расскажи как прошёл день?") is None


# ── _format_blocks ────────────────────────────────────────────────────────────

def test_format_blocks():
    out = reply_flow._format_blocks([{"observation": "наб", "mechanism": "мех", "action": "дей"}])
    assert "🔍 наб" in out and "⚙️ мех" in out and "🎯 дей" in out
