...
def _post_connect_features_text() -> str:
    ...
    return (
        "Вот что уже доступно:\n\n"
        "💬 Ответить за меня — варианты ответа под твой стиль на входящее сообщение\n"
        "📸 По скриншоту — пришли скриншот переписки → варианты ответа\n"
        "💫 Новый диалог — заходы для первого сообщения новому человеку\n"
        "🔥 Скрипты общения — как оживить разговор, без ограничений\n\n"
        f"💎 Первые {FREE_TRIAL_REQUESTS} генераций (Ответить за меня / По скриншоту / "
        "Новый диалог) — бесплатно.\n\n"
        "По подписке дополнительно откроется:\n\n"
        "🔬 Анализ собеседника — совместимость, зелёные/красные флаги, как писать "
        "этому человеку\n"
        ...
        "Как только придёт хотя бы одно сообщение из подключённого чата — внизу "
        "появится меню с кнопками.\n\n"
        "Статус подписки — /premium. Полный список команд — /help"
    )
