...
async def analyze_reply_dynamics(
    ...
    prompt = (
        ...
        "ФОРМАТ ОТВЕТА\n"
        "Максимум 3 блока. Жёсткие лимиты: observation ≤ 20 слов, mechanism ≤ 20 слов, "
        "action ≤ 30 слов. Строго теги, без текста вне них:\n\n"
...
async def rewrite_message_explained(
    ...
    prompt = (
        "Ты — уверенный дейтинг-коуч, говоришь прямо и по делу, без занудства. "
        "Помоги автору переписать черновик так, чтобы он звучал живее, увереннее и "
        "лучше цеплял конкретного собеседника — но это по-прежнему сам автор, его "
        "слова, не кто-то другой. Цель — чтобы собеседник почувствовал интерес и "
        "захотел продолжить общение.\n\n"
        f"ГОЛОС АВТОРА:\n{style_card}\n\n"
        ...
        "Затем — на «ты», уверенно и МАКСИМАЛЬНО КОРОТКО (строгий лимит: 1-2 предложения). "
        "ЧТО изменил и ПОЧЕМУ именно под этого собеседника (и под стиль, если был), "
        "со ссылкой на его привычку. Пример: «Сделал заход теплее и убрал \"Вы\" — "
        "она сама пишет на \"ты\" и коротко, длинные тексты её душнят».\n"
        "ВАЖНО: перед тем как писать пояснение, перечитай переписанное сообщение. "
        "Упоминай ТОЛЬКО те правки, которые реально есть в тексте. Если слово из черновика "
        "осталось — НЕЛЬЗЯ писать, что ты его убрал.\n"
        f"Затем строка: {_RATING}\n"
        "Затем — ОДНО короткое предложение (до 10 слов). Честная оценка, как впишется "
        "под этого собеседника. БЕЗ процентов и цифр. Начни со значка ✅ или ⚠️. "
        "Примеры: «✅ В его тоне, коротко — должно зайти» / "
        "«⚠️ Длинновато, такое часто не дочитывает»."
    )
    return _split_rated(await _ask(prompt))


async def suggest_reply(
    incoming_msg: str, style_card: str, interaction_card: str, style: str | None = None
) -> tuple[str, str, str]:
    ...
    prompt = (
        ...
        "КАК ответить так, чтобы звучать живо и уверенно — в голосе автора, с учётом "
        "привычек собеседника. Цель — чтобы собеседник почувствовал интерес и захотел "
        "продолжить общение.\n\n"
        f"ГОЛОС АВТОРА:\n{style_card}\n\n"
        ...
        f"Затем строка: {_DELIM}\n"
        "Затем — на «ты», МАКСИМАЛЬНО КОРОТКО (строгий лимит: 1-2 предложения). Почему такой "
        "ответ и как он играет на привычку собеседника. Опирайся ТОЛЬКО на текст ответа "
        "выше — не приписывай ему слов или правок, которых там реально нет.\n"
        f"Затем строка: {_RATING}\n"
        "Затем — ОДНО короткое предложение (до 10 слов). Честная оценка, как зайдёт. "
        "БЕЗ процентов. Начни со значка ✅ или ⚠️."
    )
    return _split_rated(await _ask(prompt))
...
async def suggest_reply_from_screenshot(
    ...
    prompt = (
        ...
        "скриншота, могут быть мелкие ошибки OCR). Помоги автору ответить так, чтобы "
        "звучать живо и уверенно. Цель — чтобы собеседник почувствовал интерес и "
        "захотел продолжить общение.\n\n"
        f"ГОЛОС АВТОРА:\n{style_card}\n\n"
        ...
        f"Затем строка: {_DELIM}\n"
        "Затем — на «ты», МАКСИМАЛЬНО КОРОТКО (строгий лимит: 1-2 предложения). Почему такой "
        "ответ и как он играет на привычку собеседника, простым языком. Опирайся ТОЛЬКО "
        "на текст ответа выше.\n"
        f"Затем строка: {_RATING}\n"
        "Затем — ОДНО короткое предложение (до 10 слов). Честная оценка, как зайдёт. "
        "БЕЗ процентов. Начни со значка ✅ или ⚠️."
    )
