...
def _split_deep_analysis(raw: str) -> tuple[str, str, str, str]:
    parts = _split_by_markers(raw, [_DA_HOWTO, _DA_STYLE, _DA_FLAGS])
    return parts[0], parts[1], parts[2], parts[3]
...
async def build_deep_analysis(
    ...
    """Анализ собеседника, 4 блока одним вызовом LLM: совместимость, как писать
    этому человеку (с реальными примерами удачных заходов из переписки), длина/
    ритм/регистр/язык собеседника, зелёные/красные флаги. «История по периодам»
    и «Подарки» из фичи убраны (см. v2 выше). Возвращает
    (совместимость, как_писать, стиль_собеседника, флаги)."""
    dated_lines = _fit(dated_lines)
    gender_note = _gender_note(user_gender)
    prompt = (
        "Ты — уверенный дейтинг-коуч, разбираешь переписку автора с его собеседником "
        ...
        + "\n>>>\n\n"
        "Собери ЧЕТЫРЕ блока строго в этом порядке, разделённые маркерами.\n"
        "СТИЛЬ: конкретно и по делу, без воды и без пояснений-очевидностей "
        "(«что говорит о…», «это показывает…») — но не в ущерб содержанию: там, где "
        "нужен пример или цитата, дай его полностью, не обрезай до одного слова.\n\n"
        "БЛОК 1 — Совместимость (без маркера, первым):\n"
        "• Первая строка ровно: Совместимость: XX/100 (честная оценка по динамике)\n"
        "  Шкала: 0-30 — инициатива с одной стороны, сухо; 30-60 — неровно; 60-85 — обе "
        "пишут первыми и тепло; 85-100 — почти зеркально.\n"
        ...
        "• Как часто стоит писать — честный вывод по наблюдаемому ритму его ответов "
        "(не расписание: «часто ок» / «лучше не форсировать, держать паузы» и т.п.)\n"
    ...
    raw = await _ask(prompt, max_tokens=2200)
    return _split_deep_analysis(raw)
...
async def build_deep_style_analysis(
    ...
    """Анализ своего стиля (агрегат по всем собеседникам), 3 блока одним вызовом
    ...
    фон и красные флаги, временные паттерны), короткий практический вывод.
    Совместимость с лучшим контактом — отдельный вычисляемый блок без LLM, см.
    main.py _best_compatibility_contact (этой правкой не затронут).
    Возвращает (личность, разбор, вывод)."""
    dated_lines = _fit(dated_lines)
    gender_note = _gender_note(user_gender)
    prompt = (
        ...
        + "\n".join(dated_lines)
        + "\n>>>\n\n"
        "Собери ТРИ блока строго в этом порядке, разделённые маркерами. Это "
        ...
        "диагноз\n"
        "По каждой из 5 черт — 1-2 строки, каждая строка начинается с символа "
        "табуляции и номера пункта в скобках (например «\t1) Открытость новому "
        "— ...»): черта, куда склоняется (выше среднего / ниже среднего / не "
        "считывается) и КОНКРЕТНО что в тексте на это указывает:\n"
        "\t1) Открытость новому\n"
        "\t2) Добросовестность\n"
        "\t3) Экстраверсия\n"
        ...
        "ФОРМАТ КАЖДОЙ СЕКЦИИ НИЖЕ (кроме заголовка с эмодзи): 2-3 коротких абзаца "
        "по 1-2 предложения, разделённых пустой строкой — НЕ один слитный блок "
        "текста. КАЖДЫЙ абзац начинается с символа табуляции. Пример нужного "
        "формата:\n"
        "«\tТы пишешь короткими, рублеными фразами — часто по одному-два слова.\n\n"
        "\tИзредка — целыми предложениями, но чаще это импульсивные высказывания.\n\n"
        "\tПохоже на то, что ты ценишь время в переписке больше, чем выстраивание "
        "длинных мыслей.»\n\n"
        "✍️ Стиль и структура\n"
        ...
        "❗ Пунктуация и невербальные маркеры\n"
        ...
        "🎭 Эмоциональный фон и красные флаги\n"
        ...
        "• Скорость ответов и паузы между сообщениями, судя по датам — что это "
        "может говорить о темпе принятия решений или уровне тревожности\n\n"
        f"Затем строка: {_DSA_TIP}\n"
