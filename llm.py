...
_DA_STYLE   = "===СТИЛЬ_СОБЕСЕДНИКА==="
_DA_FLAGS   = "===ФЛАГИ==="
_DA_MESSAGE = "===ГОТОВОЕ_СООБЩЕНИЕ==="


def _split_deep_analysis(raw: str) -> tuple[str, str, str, str, str]:
    parts = _split_by_markers(raw, [_DA_HOWTO, _DA_STYLE, _DA_FLAGS, _DA_MESSAGE])
...
async def build_deep_analysis(
    ...
    """Анализ собеседника, 5 блоков одним вызовом LLM: совместимость (по доле
    подхваченных бидов на контакт и взаимному самораскрытию, Готтман), как писать
    этому человеку (с таймингом и реальными примерами удачных заходов из
    переписки), длина/ритм/регистр/язык собеседника, зелёные/красные флаги
    (красные приоритизированы по тяжести: четыре всадника Готтмана первым
    пунктом, затем выраженные тревожный/избегающий паттерны привязанности, потом
    остальное — обычная сухость флагом не считается), готовое сообщение можно
    отправить прямо сейчас. «История по периодам» и «Подарки» из фичи убраны
    (см. v2 выше). Возвращает
    (совместимость, как_писать, стиль_собеседника, флаги, готовое_сообщение)."""
    dated_lines = _fit(dated_lines)
    gender_note = _gender_note(user_gender)
    prompt = (
        "Ты — уверенный дейтинг-коуч, разбираешь переписку автора с его собеседником "
        ...
        "одного слова.\n\n"
        "ПСИХОЛОГИЧЕСКАЯ РАМКА (внутренний аналитический инструмент для тебя — "
        "термины из неё в ответ не выводи, в самом выводе говори простым языком "
        "коуча, не наукообразно; на неё только опираются формулировки блоков 1 и 4):\n"
        "• Бид на контакт (bid for connection, Готтман) — любая попытка одного из "
        "собеседников получить внимание/отклик: вопрос, факт о себе, шутка, "
        "приглашение, эмоция, фото. Ответ на бид: подхватил тепло (turning toward) "
        "/ проигнорировал или ушёл в другую тему (turning away) / обесценил "
        "сарказмом или холодностью (turning against).\n"
        "• Четыре всадника (Готтман, по убыванию тяжести): презрение/обесценивание "
        "(сарказм, насмешка, снисходительный тон) — самый тяжёлый сигнал, "
        "статистически лучший предиктор охлаждения; критика личности вместо повода "
        "(«ты всегда»/«ты никогда» вместо конкретной ситуации); защита/оправдания "
        "вместо диалога по существу; стена — резкий уход от темы или продолжительное "
        "молчание именно после значимого/уязвимого сообщения.\n"
        "• Попытка примирения (repair attempt) — после трения или неловкости: "
        "шутка, извинение, тёплое сообщение, которое разряжает обстановку — сильный "
        "положительный сигнал независимо от того, что было трение.\n"
        "• Паттерн привязанности (тревожный/избегающий/стабильный, Боулби/Эйнсворт) "
        "— тревожный: эскалация тона при задержке ответа, повторные «ты где»/«всё "
        "ок?», потребность в частом подтверждении; избегающий: уход в поверхностные "
        "темы или молчание именно после момента близости/уязвимости, дискомфорт с "
        "эмоциональными темами; стабильный: ровное вовлечение и в близости, и при "
        "паузах, прямые формулировки без драмы. Это описательные паттерны, не "
        "диагноз — называй только если реально повторяется в переписке несколько "
        "раз.\n\n"
        "БЛОК 1 — Совместимость (без маркера, первым):\n"
        "• Первая строка ровно: Совместимость: XX/100 (честная оценка по динамике)\n"
        ...
        "• Затем 2-3 пункта ≤12 слов — что формирует оценку, с опорой на переписку "
        "(конкретный бид и как на него ответили, либо конкретный момент "
        "погони/избегания)\n\n"
        ...
        "• 2-3 конкретных речевых приёма или подхода, которые ему заходят — не общие "
        "слова («будь собой»), а предметно: какой заход, тон, формат работает\n"
        "• Затем 2-3 РЕАЛЬНЫХ примера из переписки: что написал автор (короткая цитата "
        ...
        f"Затем строка: {_DA_STYLE}\n"
        "БЛОК 3 — Длина, ритм, регистр и язык собеседника:\n"
        "• Типичная длина его сообщений (коротко/средне/развёрнуто, с ориентиром в "
        "словах или символах)\n"
        "• Ритм и скорость ответов\n"
        "• Регистр: на «ты»/«Вы», с большой или маленькой буквы\n"
        "• Характерные обороты, сленг, мат если есть — иначе одна строка «ничего "
        "особенного не замечено» (без выдумывания)\n\n"
        f"Затем строка: {_DA_FLAGS}\n"
        "БЛОК 4 — Зелёные и красные флаги:\n"
        "💚 Зелёные флаги — до 5 пунктов ≤15 слов, конкретно, с опорой на переписку. "
        "В приоритете (если реально нашлось): подхваченные биды, взаимное "
        "самораскрытие, попытки примирения после трения, ровное вовлечение без "
        "драмы (стабильный паттерн)\n"
        "🚩 Красные флаги — до 5 пунктов ≤15 слов, НУМЕРОВАННЫМ списком (1. 2. 3. "
        "...), конкретно, с опорой на переписку. Порядок строго по тяжести: если "
        "нашёлся паттерн уровня «четырёх всадников» (особенно презрение/"
        "обесценивание или уход от разговора после значимого сообщения) — он "
        "ВСЕГДА пункт 1, с явным названием паттерна; затем выраженный тревожный "
        "или избегающий паттерн, если реально повторяется несколько раз; обычная "
        "сухость или короткие ответы САМИ ПО СЕБЕ — это не то же самое и не тянет "
        "на флаг, не путай по тяжести с паттернами выше\n"
        ...
        "флаги, если нет отдельного паттерна из рамки выше (например мат именно как "
        "способ обесценить/унизить — это презрение, а не мат сам по себе)\n"
        "Правила по флагам (строго):\n"
        "• Честность важнее количества: если реально нашёлся один флаг или ни одного — "
        "так и покажи, НЕ выдумывай для симметрии или чтобы набрать до 5\n"
    ...
    raw = await _ask(prompt, max_tokens=2600)
    return _split_deep_analysis(raw)
...
# v1 (Голос и манера / история по периодам / SWOT / советы, 4 блока, 2 сообщения)
# — оставлено для отката. Заменено 3-блочной карточкой (архетип/факты/совет) +
# вычисляемым блоком совместимости без LLM (main.py _best_compatibility_contact).
...
# v2 (архетип/факты-и-приёмы/советы коуча, с winning_example) — оставлено
# для отката. Заменено гипотезами по Большой пятёрке + лингвистическим
# разбором (структура/пунктуация/лексика/эмоц.фон и красные флаги/темп) по
# запросу пользователя. winning_example убран — не входит в новый набор
# параметров, main._best_winning_example тоже удалена как неиспользуемая.
# _DSA_FACTS = "===ФАКТЫ==="
# _DSA_TIP   = "===СОВЕТ==="


# def _split_deep_style_analysis(raw: str) -> tuple[str, str, str]:
#     parts = _split_by_markers(raw, [_DSA_FACTS, _DSA_TIP])
#     return parts[0], parts[1], parts[2]


# async def build_deep_style_analysis(
#     dated_lines: list[str], stats_summary: str, user_gender: str | None = None,
#     winning_example: str | None = None,
# ) -> tuple[str, str, str]:
#     """Компактная карточка своего стиля (агрегат по всем собеседникам), 3 блока
#     одним вызовом LLM: архетип манеры письма, конкретные факты (коронные слова,
#     ритм, лучший ход), один практический совет. Совместимость с лучшим контактом
#     — отдельный вычисляемый блок без LLM, см. main.py _best_compatibility_contact.
#     winning_example — РЕАЛЬНАЯ цитата из features.winning_messages (не выдумана
#     моделью): в этих данных нет ответов собеседника, так что «тёплый отклик»
#     модель обязана комментировать только на уже проверенном примере, не искать
#     его сама. Возвращает (архетип, факты, совет)."""
#     dated_lines = _fit(dated_lines)
#     gender_note = _gender_note(user_gender)
#     if winning_example:
#         winning_block = (
#             "РЕАЛЬНЫЙ ПРИМЕР ТВОЕГО СООБЩЕНИЯ (уже проверено: собеседник на него "
#             f"ответил быстро и содержательно) — используй как есть, не меняй и не "
#             f"выдумывай другой:\n«{winning_example}»\n\n"
#         )
#         winning_instruction = (
#             "Процитируй пример выше и одной короткой фразой объясни, почему он "
#             "сработал"
#         )
#     else:
#         winning_block = ""
#         winning_instruction = (
#             "Явных примеров с проверенным тёплым откликом пока не набралось — "
#             "честно скажи это одной строкой, не выдумывай пример"
#         )
#     prompt = (
#         "Ты — уверенный дейтинг-коуч, разбираешь КАК этот человек пишет — все его "
#         "исходящие сообщения разным собеседникам вместе, хронологически (без привязки "
#         "к конкретному человеку). Говоришь с ним самим: на «ты», живо и по делу, "
#         "без занудства и без клинических диагнозов — только то, что реально видно "
#         "из текста. Пиши развёрнуто и содержательно, но каждый пункт — по делу, с "
#         "опорой на конкретику из переписки, без общих слов и воды.\n"
#         f"{gender_note}"
#         "Верни ТОЛЬКО текст — без JSON, без кавычек, без markdown.\n\n"
#         f"СТАТИСТИКА:\n{stats_summary}\n\n"
#         f"{winning_block}"
...
#         + "\n".join(dated_lines)
#         + "\n>>>\n\n"
#         "Собери ТРИ блока строго в этом порядке, разделённые маркерами.\n\n"
#         "БЛОК 1 — Архетип стиля (без маркера, первым):\n"
#         "• Первая строка: 🎭 [яркое название архетипа МАНЕРЫ ПИСЬМА, 2-4 слова — "
#         "не психотип и не диагноз, а именно как пишет: например «Дерзкий флиртун», "
#         "«Тёплый рассказчик»]\n"
#         "• Затем 2-3 предложения: раскрой этот архетип — какой у тебя тон, энергия, "
#         "как ты заходишь в разговор, что чувствует собеседник, читая тебя. С опорой "
#         "на конкретные обороты из переписки.\n\n"
#         f"Затем строка: {_DSA_FACTS}\n"
#         "БЛОК 2 — Твои цифры и приёмы (конкретные факты, НЕ мнения):\n"
#         "📊 Твой почерк\n"
#         "• Коронные слова/фразы — 3-5 реально повторяющихся выражений, словечек или "
#         "эмодзи из сообщений выше (НЕ выдумывать, только то, что реально встречается; "
#         "по возможности с короткой цитатой)\n"
#         "• Длина и формат — типичная длина твоих сообщений, разбиваешь ли на части, "
#         "как часто задаёшь вопросы vs утверждаешь\n"
#         "• Ритм — когда обычно пишешь и как быстро включаешься в диалог, судя по "
#         "датам сообщений\n"
#         "• Как ты флиртуешь/цепляешь — 1-2 характерных приёма, которые ты реально "
#         "используешь (юмор, поддразнивание, забота, прямота — что видно в тексте)\n"
#         f"• Лучший ход — {winning_instruction}\n\n"
#         f"Затем строка: {_DSA_TIP}\n"
#         "БЛОК 3 — Советы коуча:\n"
#         "💡 Что прокачать\n"
#         "• 3-4 конкретных практических совета для дейтинга, каждый привязан к тому, "
#         "что реально видно в твоих сообщениях (усилить сильное / поправить слабое). "
#         "Не общие слова вроде «будь собой» — предметно: что именно попробовать в "
#         "следующих переписках и зачем."
#     )
...
_DSA_FACTS   = "===ФАКТЫ==="
_DSA_TIP     = "===СОВЕТ==="
_DSA_MESSAGE = "===ПРИМЕР_СООБЩЕНИЯ==="


def _split_deep_style_analysis(raw: str) -> tuple[str, str, str, str]:
    parts = _split_by_markers(raw, [_DSA_FACTS, _DSA_TIP, _DSA_MESSAGE])
    return parts[0], parts[1], parts[2], parts[3]
...
async def build_deep_style_analysis(
    ...
    """Анализ своего стиля (агрегат по всем собеседникам), 4 блока одним вызовом
    LLM: гипотезы о чертах личности по Большой пятёрке, лингвистический разбор
    (структура фраз, пунктуация и невербальные маркеры, лексика, эмоциональный
    фон и красные флаги — без временных паттернов, убраны), короткий
    практический вывод, пример сообщения в улучшенном стиле по этому выводу.
    Совместимость с лучшим контактом — отдельный вычисляемый блок без LLM, см.
    main.py _best_compatibility_contact (этой правкой не затронут). Возвращает
    (личность, разбор, вывод, пример_сообщения)."""
    dated_lines = _fit(dated_lines)
    gender_note = _gender_note(user_gender)
    prompt = (
        ...
        "ГИПОТЕЗЫ по манере письма, а не диагноз — каждый вывод должен опираться "
        "на конкретику из текста (цитата или явный повторяющийся паттерн); если "
        "что-то не считывается — честно скажи «не считывается», не выдумывай.\n\n"
        "СТИЛЬ (строго): запрещены пункты, верные для любого человека в принципе "
        "(«ты интересная личность», «есть потенциал для роста») — каждый пункт "
        "должен опираться на конкретную цитату или паттерн из текста. Запрещены "
        "слова-заглушки без содержания сразу за ними: «это может говорить о...», "
        "«как правило...», «в целом...» — либо сразу называй конкретную причину, "
        "либо не пиши фразу вообще.\n\n"
        "КАК ВЫБИРАТЬ ПРИМЕРЫ-ЦИТАТЫ (важно, действует для ВСЕХ блоков ниже):\n"
        "✗ ПЛОХО (не используй): просто транскрипция смеха или междометия — "
        "«АХАХХА», «ну да», «окей» — это шум, а не иллюстрация. Такой пример "
        "ничего не показывает о человеке.\n"
        "✓ ХОРОШО: фраза, которая показывает характерную манеру, неожиданную "
        "деталь, конкретную реакцию или решение — то, что можно пересказать другу "
        "и он поймёт что-то о характере этого человека. Например: резкая смена "
        "темы, характерная шутка с подколом, конкретный отказ или согласие, "
        "необычное сравнение, момент явной инициативы или явного избегания.\n"
        "Если единственное что есть под рукой — это смех/междометия, ищи ДРУГОЙ "
        "момент в переписке для примера, даже если он не самый частый — редкий, "
        "но показательный пример лучше частого, но пустого. Не повторяй одну и "
        "ту же цитату (например смех «АХАХХА»/«ХАХАХА» в разных вариациях) как "
        "отдельный пример в разных местах — если уже использовал её один раз, не "
        "приводи снова как будто это новый пример.\n\n"
        "БЛОК 1 — Гипотезы о личности (без маркера, первым):\n"
        "🧬 Большая пятёрка — это гипотезы по тексту переписки, не психологический "
        "диагноз\n"
        "По каждой из 5 черт — 1-2 строки, каждая строка начинается с ПЯТИ "
        "ПРОБЕЛОВ и номера пункта в скобках (например «     1) Открытость "
        "новому — ...»): черта, куда склоняется (выше среднего / ниже среднего "
        "/ не считывается) и КОНКРЕТНО что в тексте на это указывает:\n"
        "     1) Открытость новому\n"
        "     2) Добросовестность\n"
        "     3) Экстраверсия\n"
        "     4) Доброжелательность\n"
        "     5) Нейротизм (эмоциональная устойчивость)\n\n"
        "ВАЖНО про Нейротизм: частое использование мата, капса или грубой лексики "
        "САМО ПО СЕБЕ не означает нейротизм — это просто экспрессивный разговорный "
        "регистр речи, упомяни его в секции «Лексика»/«Пунктуация», а не как "
        "признак эмоциональной нестабильности. Нейротизм подтверждай ТОЛЬКО "
        "реальными эмоциональными сигналами: явное раздражение, тревога, резкие "
        "эмоциональные реакции на конкретные события, жалобы на состояние — не "
        "просто грубые слова. Если таких сигналов в переписке нет — честно пиши, "
        "что нейротизм не считывается или в норме, не притягивай мат как "
        "доказательство.\n\n"
        f"Затем строка: {_DSA_FACTS}\n"
        "БЛОК 2 — Как ты пишешь на самом деле (конкретные наблюдения, НЕ мнения):\n"
        "ФОРМАТ КАЖДОЙ СЕКЦИИ НИЖЕ (кроме заголовка с эмодзи): короткие "
        "пронумерованные пункты (1), 2) и т.д.), КАЖДЫЙ начинается с ПЯТИ "
        "ПРОБЕЛОВ перед номером, пункты разделены пустой строкой — НЕ слитный "
        "текст. Количество пунктов на секцию указано в её описании ниже. "
        "Пример нужного формата:\n"
        "«     1) Ты пишешь короткими, рублеными фразами — часто по "
        "одному-два слова.\n\n"
        "     2) Изредка — целыми предложениями, но чаще это импульсивные "
        "высказывания.»\n\n"
        "✍️ Стиль и структура (1-2 пункта, коротко)\n"
        "• Короткие рубленые фразы или длинные сложные предложения — и что это "
        "обычно означает (ценность времени/импульсивность vs эмоциональность/"
        "склонность к рефлексии)\n\n"
        "❗ Пунктуация и невербальные маркеры (1-2 пункта, коротко)\n"
        "• Что реально встречается — восклицательные знаки, многоточия, КАПС, "
        "игнорирование знаков препинания, обилие эмодзи или их полное отсутствие "
        "— и о чём это обычно говорит (эмоциональность, задумчивость/неуверенность, "
        "желание привлечь внимание, торопливость и т.п.)\n\n"
        "🗣️ Лексика (1-2 пункта, коротко)\n"
        "• Какие слова и темы доминируют; частота личных местоимений («я»/«мне»); "
        "слова-маркеры вроде «на самом деле», «по сути», «в общем», если реально "
        "встречаются — и что это может значить\n\n"
        "🎭 Эмоциональный фон и красные флаги (2-3 пункта)\n"
        "• Как меняется тон между разными темами; есть ли резкие смены стиля "
        "(например сарказм в ответ на критику) — конкретный пример, если нашёлся. "
        "Явного паттерна нет — так и скажи, не выдумывай\n\n"
        f"Затем строка: {_DSA_TIP}\n"
