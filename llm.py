...
async def suggest_reply_variants(
    ...
    if previous_variants:
        ...
        regen_block = (
            ...
            "другими словами, а другие углы (например, если раньше был прямой и "
            "мягкий — попробуй с юмором и с любопытством). Названия и содержание "
            "не должны пересекаться с уже показанными.\n\n"
        )
    prompt = (
        ...
        f"мысли другими словами, а реально разные СТРАТЕГИИ ответа (например: "
        f"прямой и уверенный / тёплый и мягкий / с лёгким юмором — либо другой "
        f"набор подходов, если он лучше подходит именно этому сообщению). "
        ...
        "=== НАЗВАНИЯ ВАРИАНТОВ ===\n"
        "Короткое название (2-4 слова), которое отражает СУТЬ подхода именно для "
        "этого сообщения — не общая метка «стиль 1», а конкретная характеристика "
        "(например «прямой и уверенный», «с лёгким юмором», «тёплый и без "
        "давления», «с любопытством»). Названия не должны повторяться и не "
        "обязаны совпадать с готовым списком стилей бота — выбирай то, что "
        "точнее всего описывает конкретный вариант.\n\n"
        "=== ЯЗЫК (строго) ===\n"
...
async def screenshot_variants(
    ...
    prompt = (
        ...
        f"одной мысли, а реально разные СТРАТЕГИИ (например: прямой и "
        f"уверенный / тёплый и мягкий / с лёгким юмором — либо другой набор "
        f"подходов, если он лучше подходит именно этому сообщению). Каждый "
        ...
        "=== НАЗВАНИЯ ВАРИАНТОВ ===\n"
        "Короткое название (2-4 слова), отражающее СУТЬ подхода именно для "
        "этого сообщения — конкретная характеристика, не общая метка. "
