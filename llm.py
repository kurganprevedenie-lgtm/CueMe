...
async def build_deep_analysis(
    ...
    """Анализ собеседника, 5 блоков одним вызовом LLM, вдвое компактнее прежней
    версии: совместимость (1-2 пункта строго в формате «бид → ответ», без общих
    фраз про переписку целиком), как писать этому человеку (тайминг + 1-2
    приёма + 1-2 примера), длина/ритм/регистр/язык собеседника (3 строки),
    зелёные/красные флаги (максимум по 3, оба цвета нумерованные, КАЖДЫЙ пункт
    обязан ссылаться на конкретную цитату/момент — иначе не считается флагом;
    красные приоритизированы по тяжести: четыре всадника Готтмана пунктом 1),
    готовое сообщение можно отправить прямо сейчас. «История по периодам» и
    «Подарки» из фичи убраны (см. v2 выше). Возвращает
    ...
    prompt = (
        ...
        "• Затем 1-2 пункта ≤10 слов СТРОГО в формате «Бид: [что написал] → "
        "Ответ: [как отреагировал]» или «Момент погони/избегания: [что было]» — "
        "без общих фраз про переписку целиком («переписка рабочая», «мало "
        "романтики») — это не объясняет оценку, а только конкретный бид/момент "
        "и реакция на него объясняют\n\n"
        ...
        "БЛОК 3 — Длина, ритм, регистр и язык собеседника (3 строки, каждая ≤15 "
        "слов):\n"
        "• Строка 1: типичная длина сообщений + скорость ответов одной фразой\n"
        "• Строка 2: регистр — «ты»/«Вы», с большой или маленькой буквы\n"
        "• Строка 3: характерные обороты/сленг/мат если реально есть — иначе "
        "«ничего особенного не замечено» (без выдумывания)\n\n"
        f"Затем строка: {_DA_FLAGS}\n"
        ...
        "лучше...») — это ОТВЕТ по существу, а не уход, флагом не считается; "
        "флаг ставь только когда вопрос реально проигнорирован или тема сменилась "
        "без всякой реакции на него\n"
