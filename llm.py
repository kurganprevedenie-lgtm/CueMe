...
def _parse_axis_block(raw: str, label: str, default: int = 2) -> tuple[int, str]:
    ...
    и обрезаем — защита формата на стороне Python, а не только промпта."""
...
async def build_deep_analysis(
    ...
    prompt = (
        ...
        "факт, и он честнее круглой отговорки.\n"
        "• Строки с пометкой «[шум, не цитировать]» — смех, междометия, голая "
        "пунктуация, куцые реакции без контекста («сукаааа», «лол», «чо») — "
        ...
        "ИНТЕРЕС_B: <0-5> | <есть ли случаи, где собеседник САМ делится о себе "
        "БЕЗ вопроса от автора — взаимность, а не только ответы на вопросы; "
        "приведи цитату такого момента или точное число случаев (в т.ч. 0)>\n"
        "ЮМОР: <0-5> | <найди пары «шутка автора → реакция собеседника»: "
        "сколько их и была ли встречная шутка/живой отклик, а не сухой ответ; "
        "с цитатой хотя бы одной пары, если она есть>\n"
        "ПЛАНЫ: <0-5> | <упоминания совместных планов любого рода (не только "
        "офлайн-встреча — «посмотрим фильм», «сходим», «съездим» тоже "
        "считаются): сколько их, конкретика (дата/место/формат) или "
        "уклончивость («может быть», «посмотрим»), с цитатой примера>\n\n"
        ...
        "ВСЕХ пяти сигналов выше (трёх готовых фактов и трёх твоих оценок)>"
    )
    raw = await _ask(prompt, max_tokens=1400)
