...
async def rewrite_message_explained(
    ...
    prompt = (
        ...
        "ЧТО изменил и ПОЧЕМУ именно под этого собеседника (и под выбранный стиль, если он был), "
        "простым языком, со ссылкой на его привычку. "
        ...
        "ВАЖНО: перед тем как писать это пояснение, ещё раз перечитай переписанное сообщение "
        "СЛОВО В СЛОВО. Упоминай в пояснении ТОЛЬКО те правки, которые реально видны в тексте "
        "выше. Если слово или фраза из черновика осталась в переписанном сообщении — "
        "НЕЛЬЗЯ писать, что ты её убрал или заменил.\n"
...
async def suggest_reply(
    ...
    prompt = (
        ...
        "собеседника (и на выбранный стиль, если он был), простым языком. Опирайся ТОЛЬКО "
        "на текст ответа выше — не приписывай ему слов или правок, которых там реально нет.\n"
        f"Затем строка: {_RATING}\n"
...
async def suggest_reply_from_screenshot(
    ...
    prompt = (
        ...
        "на привычку собеседника, простым языком. Опирайся ТОЛЬКО на текст ответа выше — "
        "не приписывай ему слов или правок, которых там реально нет.\n"
        f"Затем строка: {_RATING}\n"
        "Затем — короткая честная оценка, как зайдёт. БЕЗ процентов, по-русски. "
        "Начни со значка ✅ или ⚠️."
    )
