...
from features import ChatFeatures, initiative_axis, interest_signal_a, response_speed_axis
...
def _parse_axis_score(raw: str, label: str, default: int = 2) -> int:
    """Достаёт целое 0-5 после метки «label:» из ответа LLM. Не нашли/не
    распарсили — нейтральный default, а не падение всей фичи."""
    m = re.search(rf"{re.escape(label)}\s*:\s*([0-5])", raw)
    return int(m.group(1)) if m else default


def _parse_da_verdict(raw: str) -> str:
    """Текст после «ВЫВОД:» — обрезаем до 2 предложений на стороне Python
    (защита формата, а не только промпт: это платное ядро фичи)."""
    m = re.search(r"ВЫВОД\s*:\s*(.+)", raw, re.S)
    text = (m.group(1) if m else raw).strip()
    text = re.split(r"\n\s*\n", text)[0].strip()  # отсечь, если LLM всё же добавил что-то ещё блоком ниже
...
async def build_deep_analysis(
    ...
    5 осей (0-5 каждая, сумма 0-25 + медаль) вместо единого числа 0-100 — цель:
    бороться с фиктивной «объективностью» единого score, когда за красивым
    числом на деле стоит непрозрачный вайб модели (см. критику 90-балльных
    систем рейтинга а-ля RateYourMusic). Три оси считаются программно в
    features.py БЕЗ LLM (инициативность — кто чаще пишет первым после паузы
    ≥4ч; сигнал A интереса — доля вопросов-обращений «ты/тебе»; скорость
    ответов — средняя латентность + штраф за необъяснённые долгие паузы), две
    с половиной — LLM, но СТРОГО с опорой на конкретную цитату/момент (сигнал
    B интереса — самораскрытие без вопроса; юмор — пара маркер→реакция; общие
    планы — конкретика vs. уклончивость); если опереться не на что — оценка
    честно низкая/средняя, а не «на глаз». Python сам собирает медаль, сумму и
    строку осей — LLM не доверяется арифметика и форматирование заголовка.
    Блоки 2-4 (как писать / флаги / готовое сообщение) не изменены. Возвращает
    (совместимость, как_писать, флаги, готовое_сообщение)."""
    axis1_score, _ = initiative_axis(rows)
    axis2a_score, _ = interest_signal_a(rows)
    axis5_score, _ = response_speed_axis(rows)
    ...
    prompt = (
        ...
        "близости, и в паузах). Не диагноз — упоминай, только если реально "
        "повторяется несколько раз.\n\n"
        "БЛОК 1 — три оси и вывод (без маркера, первым). Инициативность, "
        "интерес(сигнал A) и скорость ответов уже посчитаны программно вне "
        "тебя — ты оцениваешь ТОЛЬКО три вещи ниже, строго целым числом 0-5 "
        "каждая, СТРОГО с опорой на конкретную цитату/момент («на глаз» "
        "оценивать запрещено — если опереться не на что, оценка честно "
        "низкая/средняя, и это должно быть видно из пояснения):\n"
        "ИНТЕРЕС_B: <0-5> | <цитата/момент, где собеседник САМ делится о себе "
        "БЕЗ вопроса от автора — взаимность, а не только ответы на вопросы; "
        "если такого не нашлось — честно «не найдено», оценка 1-2>\n"
        "ЮМОР: <0-5> | <конкретная пара «сообщение автора с шуткой → реакция "
        "собеседника» с обеими цитатами; если явных содержательных шуток мало "
        "или нет (голый смех «хаха» без содержания НЕ считается) — честно "
        "«пар мало», оценка 2-3>\n"
        "ПЛАНЫ: <0-5> | <цитата про совместные планы любого рода (не только "
        "офлайн-встреча — «посмотрим фильм», «сходим», «съездим» тоже "
        "считаются) и есть ли в ней конкретика (дата/место/формат) против "
        "уклончивости («может быть», «посмотрим»); если планы не обсуждались "
        "— честно «не обсуждались», оценка 1>\n"
        "ВЫВОД: <строго 1-2 предложения живым языком с ОДНОЙ конкретной "
        "деталью или цитатой из переписки, которую сами цифры не передают — "
        "не пересказывай, что означает каждая цифра, не используй "
        "слова-заглушки>\n\n"
        f"Затем строка: {_DA_HOWTO}\n"
        "БЛОК 2 — Как писать этому человеку:\n"
        "• Тайминг — по времени в переписке найди реальный паттерн (например: "
    ...
    axis2b_score = _parse_axis_score(block1_raw, "ИНТЕРЕС_B")
    axis3_score  = _parse_axis_score(block1_raw, "ЮМОР")
    axis4_score  = _parse_axis_score(block1_raw, "ПЛАНЫ")
    verdict_text = _parse_da_verdict(block1_raw)
    ...
    compat = (
        f"{medal} — {total}/25\n\n"
        f"Инициативность: {axis1_score}/5 · Интерес: {axis2_score}/5 · "
