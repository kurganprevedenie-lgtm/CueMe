...
async def build_deep_style_analysis(
    ...
    prompt = (
        ...
        "без занудства и без клинических диагнозов — только то, что реально видно "
        "из текста. Карточка компактная — читается за 20-30 секунд, без воды.\n"
        f"{gender_note}"
        "Верни ТОЛЬКО текст — без JSON, без кавычек, без markdown.\n\n"
        f"СТАТИСТИКА:\n{stats_summary}\n\n"
        f"{winning_block}"
        ...
        + "\n".join(dated_lines)
        + "\n>>>\n\n"
        "Собери ТРИ блока строго в этом порядке, разделённые маркерами. Каждый "
        "пункт — короткая фраза 10-15 слов, без воды и без пояснений-очевидностей.\n\n"
        ...
        "«Тёплый рассказчик»]\n"
        "• Вторая строка: [1 короткая фраза ≤15 слов — что в тексте на это указывает]\n\n"
        f"Затем строка: {_DSA_FACTS}\n"
        "БЛОК 2 — Твои цифры (конкретные факты, НЕ мнения):\n"
        "📊 Твои цифры\n"
        "• Коронные слова/фразы — 2-3 реально повторяющихся выражения или эмодзи "
        "из сообщений выше (НЕ выдумывать, только то, что реально встречается)\n"
        "• Ритм — когда обычно пишешь, судя по датам сообщений (одна строка)\n"
        f"• Лучший ход — {winning_instruction}\n\n"
        f"Затем строка: {_DSA_TIP}\n"
        "БЛОК 3 — Один совет:\n"
        "• Выведи РОВНО одну строку в формате: 💡 [ОДНА конкретная практическая "
        "рекомендация для дейтинга, ≤20 слов, опирается на то, что видно в тексте — "
        "не список, один ударный совет]"
    )
