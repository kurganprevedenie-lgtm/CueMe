...
async def rewrite_message_explained(
    ...
    prompt = (
        ...
        "• Не начинай со штампованных зачинов «слушай,», «кстати,», «честно "
        "говоря,» — если можно живее, начни иначе.\n\n"
...
async def suggest_reply(
    ...
    prompt = (
        ...
        "который переводит разговор в конструктивное русло.\n"
        ...
        "• Без навязчивости и заискивания: интерес с самоуважением, не снизу. Не "
        "начинай со штампов «слушай,», «кстати,», «честно говоря,».\n\n"
        ...
        "• Если сообщение — отказ, холод, сарказм или грубость: отвечай с "
        "достоинством. Не дожимай, не оправдывайся, не груби в ответ; легко и "
        "спокойно оставь дверь открытой или красиво отступи.\n\n"
        "=== ЯЗЫК ПОЯСНЕНИЯ И ОЦЕНКИ (строго) ===\n"
        "• Пиши ТОЛЬКО по-русски, простыми словами. Никаких английских слов.\n"
        ...
        "Почему такой ответ и как он играет на привычку собеседника. Опирайся "
        "ТОЛЬКО на текст ответа выше — не приписывай ему слов или правок, которых "
        "там реально нет.\n"
        f"Затем строка: {_RATING}\n"
        "Затем — ОДНО короткое предложение (до 10 слов). Честная оценка, как "
...
async def suggest_reply_from_screenshot(
    ...
    prompt = (
        ...
        "• Если разговор идёт живо и долго и тон тёплый — уместно мягко предложить "
        "перевести общение в оффлайн (встречу), без форсирования и давления.\n"
        "• Если последняя реплика — отказ, холод, сарказм или грубость: отвечай с "
        "достоинством. Не дожимай, не оправдывайся, не груби в ответ; легко и "
        "спокойно оставь дверь открытой или красиво отступи.\n\n"
        "=== ЯЗЫК ПОЯСНЕНИЯ И ОЦЕНКИ (строго) ===\n"
        "• Пиши ТОЛЬКО по-русски, простыми словами. Никаких английских слов.\n"
        ...
        f"Затем строка: {_DELIM}\n"
        "Затем — на «ты», МАКСИМАЛЬНО КОРОТКО (строгий лимит: 1-2 предложения). "
        "Почему такой ответ и как он играет на привычку собеседника, простым "
        "языком. Опирайся ТОЛЬКО на текст ответа выше.\n"
        f"Затем строка: {_RATING}\n"
        "Затем — ОДНО короткое предложение (до 10 слов). Честная оценка, как "
        "зайдёт. БЕЗ процентов. Начни со значка ✅ или ⚠️; если ⚠️ — в тех же "
        "словах дай микро-фикс (что подправить)."
