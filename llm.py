...
async def build_deep_style_analysis(
    ...
    практический вывод. Совместимость с лучшим контактом — отдельный
    вычисляемый блок без LLM, см. main.py _best_compatibility_contact (этой
    правкой не затронут). Возвращает (личность, разбор, вывод)."""
    dated_lines = _fit(dated_lines)
    gender_note = _gender_note(user_gender)
    prompt = (
