...
async def build_deep_analysis(
    ...
    prompt = (
        ...
        "🚩 Красные флаги — до 5 пунктов ≤15 слов, конкретно, с опорой на переписку. "
        "Порядок строго по тяжести: если нашёлся паттерн уровня «четырёх всадников» "
        "(особенно презрение/обесценивание или уход от разговора после значимого "
        "сообщения) — он ВСЕГДА первым пунктом, с явным названием паттерна; затем "
