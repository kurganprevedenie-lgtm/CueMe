...
async def build_deep_analysis(
    ...
    версии: совместимость (тезисно, БЕЗ технических ярлыков «Бид:»/«Ответ:» —
    либо «Раньше/Сейчас» при длинной истории с заметным сдвигом, либо
    заголовок-вывод + одна строка с «→» и конкретным фактом), как писать этому
    человеку (тайминг + 1-2 приёма + 1-2 примера), длина/ритм/регистр/язык
    собеседника (3 строки), зелёные/красные флаги (максимум по 3, оба цвета
    нумерованные, КАЖДЫЙ пункт обязан ссылаться на конкретную цитату/момент —
    иначе не считается флагом; красные приоритизированы по тяжести: четыре
    всадника Готтмана пунктом 1; зелёные и красные не должны противоречить друг
    другу по одной теме — сводятся в один нюансированный флаг), готовое
    сообщение можно отправить прямо сейчас. «История по периодам» и «Подарки»
    из фичи убраны (см. v2 выше). Возвращает
