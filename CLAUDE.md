...
4. **Глубокий анализ** (кнопка «🔬 Глубокий анализ»):
   - Анализ совместимости (0-100) с описанием динамики
   - История отношений по периодам с оценкой тона
   - Сильные стороны / возможные проблемы / возможности для роста
   - Рекомендации подарков на основе интересов собеседника из переписки
...
2. **Telegram Business API** (в приложении: Настройки → Telegram для бизнеса →
   Чат-боты → «Ответы на сообщения»; со стороны бота — Secretary Mode в BotFather):
   бот получает живой поток всех сообщений подключённых чатов в реальном времени,
   сохраняет в business_messages. Работает без Premium (с обновлением май 2026).
   Инструкция в боте (`_business_connect_text`) — с учётом платформы (iPhone/
   Android/десктоп отличаются только тем, как открыть Настройки).
...
4. Глубокий анализ → одним LLM-вызовом, кэш в deep_analysis
...
users(telegram_id PK, my_id, created_at, auto_mode, auto_contact_id,
      last_style_rebuild_count, trial_used)
...
- Все остальные функции (глубокий анализ, глубокий анализ стиля, стиль
