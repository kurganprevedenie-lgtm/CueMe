...
4. **Анализ собеседника** (кнопка «🔬 Анализ собеседника»), 4 блока (блок
   «Длина/ритм/язык» убран целиком — недостаточно интересно читать). Совместимость
   и флаги построены на психологической рамке (Готтман: биды на контакт/четыре
   всадника/попытки примирения; Боулби-Эйнсворт: тревожный/избегающий/стабильный
   паттерн привязанности) — модель использует её как внутренний инструмент
   анализа, термины (включая «Бид»/«Ответ») в вывод не идут:
   - Совместимость (0-100, не круглое число) — первая строка сразу с ярким
     вердиктом («22/100 — держит на расстоянии»), затем 2 строки живым тезисным
     языком («Раньше/Сейчас» при заметном сдвиге или «заголовок + факт»), без
     общих фраз про переписку целиком
   - Как писать этому человеку — тайминг по реальным паттернам переписки, приёмы,
     реальные примеры удачных заходов из переписки
   - 💚 Зелёные / 🚩 Красные флаги — максимум по 3, оба нумерованные, каждый
     пункт обязан ссылаться на конкретную цитату (иначе не считается флагом —
     фильтр против того, чтобы модель придумывала зелёные флаги «для количества»);
     разовая вежливая граница («не хочу») не флаг ни под каким предлогом, дружеский
     стёб — не презрение; самый тяжёлый красный паттерн — всегда пункт 1; зелёные
     и красные по одной теме не должны противоречить друг другу
   - ✉️ Готовое сообщение — можно скопировать и отправить прямо сейчас
     (отдельным tap-to-copy сообщением)

5. **Анализ своего стиля** (кнопка «🪞 Анализ своего стиля») — гипотезы о личности
   по Большой пятёрке, лингвистический разбор (структура фраз, пунктуация,
   лексика, эмоциональный фон), один практический совет, пример сообщения по
   этому совету. Плюс отдельный вычисляемый блок «лучшая совместимость» —
   из уже посчитанных «Анализ собеседника» по другим контактам, без LLM.

6. **Скриншот → ответ** (кнопка «📸 По скриншоту») — Vision-модель читает скриншот
...
deep_analysis(contact_id PK, compatibility_text, howto_text,
              flags_text, message_text, updated_at)
deep_style_analysis(user_telegram_id PK, profile_text, facts_text,
                    tip_text, message_text, updated_at)
