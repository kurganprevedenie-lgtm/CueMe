...
4. **Анализ собеседника** (кнопка «🔬 Анализ собеседника»), 5 блоков, вдвое
   компактнее прежней версии. Совместимость и флаги построены на психологической
   рамке (Готтман: биды на контакт/четыре всадника/попытки примирения;
   Боулби-Эйнсворт: тревожный/избегающий/стабильный паттерн привязанности) —
   модель использует её как внутренний инструмент анализа, термины в вывод не
   идут:
   - Совместимость (0-100, не круглое число) — 1-2 пункта строго в формате
     «Бид → Ответ», без общих фраз про переписку целиком
   - Как писать этому человеку — тайминг по реальным паттернам переписки, приёмы,
     реальные примеры удачных заходов из переписки
   - Длина, ритм, регистр и язык собеседника (3 строки)
   - 💚 Зелёные / 🚩 Красные флаги — максимум по 3, оба нумерованные, каждый
     ...
     самый тяжёлый красный паттерн — всегда пункт 1
...
deep_analysis(contact_id PK, compatibility_text, howto_text,
              style_text, flags_text, message_text, updated_at)
