...
- LLM: каскад с fallback — **Gemini** (`gemini-flash-lite-latest`, живой
  алиас, основной) → **Groq** (`openai/gpt-oss-120b`) → **Cloudflare Workers
  ...
  **Cerebras** (`llama-3.3-70b`, бесплатный тир, опционален) → **Mistral**
  ...
  выше (проверено вживую через `tools/check_keys.py`). 2026-09-22:
  gemini-flash-latest словил HTTP 503 «high demand» у Google (перегрузка
  именно этой модели) — переключились на gemini-flash-lite-latest (свой
  отдельный пул мощностей, ответы попроще). См. GeminiProvider в llm.py,
  если понадобится вернуть обратно. gpt-oss — reasoning-
...
-- deep_style_analysis: таблица осталась в схеме (миграция не дропала),
...
## Устоявшиеся конвенции (из практики правок)

Ниже — паттерны, которые видны по всей истории промптов и кода проекта
(PROMPTS.md, llm.py, compatibility_metrics.py, features.py) и должны
соблюдаться в новых правках. Правило «plain text, без JSON» в LLM-выводе —
см. «Соглашения по коду» выше, здесь не повторяется.

- **Не удалять старые версии логики/промптов физически — комментировать с
  пояснением, зачем/почему сменили.** При каждой существенной смене подхода
  (новая система метрик, новый формат карточки, другой промпт) предыдущая
  версия остаётся закомментированной в файле с докстрингом «что было и
  почему заменили», пометкой вида «оставлено для отката» (много примеров в
  llm.py/PROMPTS.md — старые версии `build_deep_analysis`,
  `build_deep_style_analysis`, 5-осевая система оценки и т.п.). Это касается
  содержательных итераций логики, а не каждой мелкой правки; прямой запрос
  пользователя «убери совсем» отменяет правило для конкретного случая.
- **Формат карточек в Telegram**: эмодзи-заголовок на каждую секцию (🧬 ✍️ ❗
  🗣️ 🎭 💡 💚 🚩 и т.п.), секции разделены пустой строкой, никакого markdown
  и JSON в тексте, который видит пользователь — единый ЯЗЫК ПОЯСНЕНИЯ И
  ОЦЕНКИ во всех промптах генерации (PROMPTS.md, Части 1 и 2).
- **Детерминированные метрики + реальные цитаты вместо абстрактных выводов.**
  Где можно посчитать кодом (compatibility_metrics.py/features.py) — считаем
  без LLM; LLM только интерпретирует уже посчитанный факт, не изобретает
  числа заново (`build_compatibility_interpretation` получает на вход
  fact-строки, не сырую переписку). Каждое наблюдение/флаг обязано ссылаться
  на конкретную цитату или повторяющийся паттерн — общая фраза без примера
  не считается (правило PROMPTS.md: «если пункт нельзя опровергнуть данными
  этой конкретной переписки, его не должно быть»). Цитаты фильтруются от
  мусора (`_looks_junky` в features.py / пометка «[шум, не цитировать]» в
  промпте) — смех, междометия, голая пунктуация в пример не годятся.
- **Честность важнее полноты.** Если данных мало или паттерн не считывается —
  так и написать («не считывается», «явного случая не нашлось»), не
  выдумывать ради заполнения секции или симметрии зелёных/красных
  флагов. Лучше показать меньше пунктов, но каждый обоснованный.
- **Сначала исследовать репозиторий и показать что нашёл — потом писать
  код.** Перед правкой — найти и явно показать, где сейчас реализована
  логика, какая структура данных, кто вызывающий код. Расхождения между тем,
  что предполагает формулировка задачи, и тем, что реально в коде,
  называть прямо, а не молча подгонять реализацию под ошибочное
  предположение.
- **Язык — только русский** во всём, что видит пользователь (текст бота,
  пояснения, оценки): никаких английских слов, иероглифов, другой
  латиницы/кириллицы вперемешку. Техническая кухня (`interaction_card`,
  `style_card`, названия секций анализа, слово «раздел») в пользовательский
  текст не попадает — «говори по-человечески» (дословная формулировка из
  промптов, PROMPTS.md).
